...
function initMobileTabs(){
  ...
  function show(which){
    ...
    if (which === "diagram") { center && center.classList.add("mobileOn"); setActive(bDiagram); setTimeout(()=>renderAll(), 0); }
...
function focusGroup(prefix){
  ...
  // If diagram is hidden (mobile Stages tab) or hotspots not built yet,
  // defer the focus until Diagram pane is visible and hotspots are rendered.
  if (img.clientWidth === 0 || polyById.size === 0){
    pendingGroup = prefix;
...
function renderAll(){
  ...
  // Execute deferred focus (mobile) after hotspots are ready
  if (pendingGroup){
    const g = pendingGroup;
    pendingGroup = null;
    setTimeout(()=>focusGroup(g), 0);
  }
}
